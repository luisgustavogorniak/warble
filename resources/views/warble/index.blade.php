<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('warble.store') }}">
            @csrf
            ...
            <x-primary-button class="mt-4">{{ __('Warble') }}</x-primary-button>
        </form>
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <small class="ml-2 text-sm text-gray-600">{{ $warble->created_at->format('j M Y, g:i a') }}</small>
                        ...
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('warble.edit', $warble)">
                                    {{ __('Edit') }}
                                </x-dropdown-link>
                            </x-slot>
                        </x-dropdown>
                    ...
                    <p class="mt-4 text-lg text-gray-900">{{ $warble->message }}</p>
                </div>
            </div>
            @endforeach
        </div>
